...
public class GopherEntity
{
    ...
    public bool IsInterface =>
        Type is PhoneBookTypeChar or ErrorTypeChar or TelnetTypeChar or InfoTypeChar or Telnet3270TypeChar;

    public bool IsBinary => Type is DosBinaryTypeChar or BinaryTypeChar or GifTypeChar or ImageTypeChar
        or BitmapTypeChar or MovieTypeChar or SoundTypeChar or DocTypeChar or WavTypeChar or PngTypeChar
        or RichTextTypeChar or PdfTypeChar or XmlTypeChar;

    public bool IsEncodedText => Type is BinHexTypeChar or UuencodedTypeChar;
    ...
    public GopherEntity(string entityString)
    {
        ...
        var parts = entityString.Split(['\t'], 5);
        switch (parts.Length)
        {
            case 5:
                ignore = parts[4];
    ...
    public GopherEntity(string uriString, string displayText)
    {
        ...
        Init(new Uri(uriString ?? string.Empty), displayText);
    }
    ...
    {
        return query is { Length: > 1 } && query[0] == '#'
            ? query[1..]
            : query;
    }
    ...
    {
        return selector.StartsWith("url:", StringComparison.InvariantCultureIgnoreCase)
            ? selector[4..]
            : selector;
    }
    ...
    public Uri ToUri()
    {
        ...
        var extraValue = string.IsNullOrWhiteSpace(SearchTerms) ? null : $"#{SearchTerms}";
